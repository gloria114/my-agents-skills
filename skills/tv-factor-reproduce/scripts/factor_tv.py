#!/usr/bin/env python3
"""Deterministic production core for the locked 142 TradingView factors.

The caller must provide one chronologically ordered, continuous
symbol-by-timeframe history. Warmup rows belong in the input; this module
never reads contracts, qdh, ClickHouse, or files at runtime.
"""
...
COLUMN_ORDER = (
    "tv_squeeze_momentum_lb",
    "tv_squeeze_on",
    "tv_squeeze_off",
    "tv_supertrend_14_4_line",
    "tv_supertrend_14_4_dir",
    "tv_williams_vix_fix_22",
    "tv_williams_vix_fix_upper_band",
    "tv_wavetrend_wt1_10_21",
    "tv_wavetrend_wt2_4",
    "tv_wavetrend_hist",
    "tv_utbot_trailing_stop_10_1",
    "tv_utbot_signal_10_1",
    "tv_macd_custom_macd_12_26",
    "tv_macd_custom_signal_9",
    "tv_macd_custom_hist",
    "tv_relative_volume_20",
    "tv_ema_8",
    "tv_ema_21",
    "tv_ema_34",
    "tv_ema_55",
    ...
    "tv_psar_002_02",
    "tv_psar_dir",
    "tv_supertrend_10_3_line",
    "tv_supertrend_10_3_dir",
    "tv_supertrend_21_3_line",
    "tv_supertrend_21_3_dir",
    "tv_ichimoku_tenkan_9",
    "tv_ichimoku_kijun_26",
    "tv_ichimoku_span_a_raw",
    "tv_ichimoku_span_b_raw",
    "tv_atr_14",
    "tv_atr_pct_14",
    "tv_natr_14",
    "tv_bb_20_basis",
    ...
    "tv_kc_20_lower",
    "tv_kc_20_width",
    "tv_donchian_20_upper",
    "tv_donchian_20_lower",
    "tv_donchian_20_mid",
    "tv_donchian_55_upper",
    "tv_donchian_55_lower",
    "tv_donchian_55_mid",
    "tv_chandelier_long_22_3",
    "tv_chandelier_short_22_3",
    "tv_historical_volatility_20",
    "tv_historical_volatility_60",
    "tv_ulcer_index_14",
    "tv_true_range_pct",
    "tv_range_percentile_20",
    "tv_rsi_7",
    "tv_rsi_14",
    "tv_rsi_21",
    "tv_rsi_14_ma_9",
    ...
    "tv_roc_20",
    "tv_momentum_10",
    "tv_awesome_oscillator",
    "tv_accelerator_oscillator",
    "tv_ppo_12_26",
    "tv_ppo_signal_9",
    "tv_ppo_hist",
    "tv_zero_lag_macd",
    ...
    "tv_vwap_distance_20",
    "tv_volume_zscore_20",
    "tv_dollar_volume_ma20",
    "tv_relative_volume_50",
    "tv_uptick_volume_ratio_20",
    "tv_candle_body_pct",
    "tv_candle_upper_wick_pct",
    "tv_candle_lower_wick_pct",
    "tv_close_location_value",
    "tv_gap_pct",
    "tv_intrabar_return",
    "tv_trend_bar_up",
    "tv_inside_bar",
    "tv_outside_bar",
    "tv_breakout_high_20_prev",
    "tv_breakdown_low_20_prev",
    "tv_consecutive_up_bars",
    "tv_consecutive_down_bars",
)

__all__ = ["BASE_COLUMNS", "COLUMN_ORDER", "CUMULATIVE_COLUMNS", "compute"]
...
def parabolic_sar(h: pd.Series, l: pd.Series, c: pd.Series) -> tuple[pd.Series, pd.Series]:
    ...
    # Causal cold-start seed: row 0 may not inspect close[1] or any future bar.
    # Warmup histories naturally move this bootstrap state out of the live slice.
    long_pos = c.iloc[0] >= (h.iloc[0] + l.iloc[0]) / 2.0
...
def _compute_core(df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    # Absolute cumulative levels are anchored only once, at the earliest row
    # of the full history supplied by the caller.
    sign_change = np.sign(c.diff().fillna(0))
    obv_increments = sign_change * v
    if len(obv_increments):
        obv_increments.iloc[0] = 0.0
    obv = obv_increments.cumsum()
    calc["tv_obv"] = obv
    calc["tv_obv_ema_20"] = ema(obv, 20)

    raw_clv = ((c - l) - (h - c)) / (h - l).replace(0, np.nan)
    money_flow_volume = raw_clv.fillna(0) * v
    adl_increments = money_flow_volume.copy()
    if len(adl_increments):
        adl_increments.iloc[0] = 0.0
    adl = adl_increments.cumsum()
    calc["tv_adl"] = adl
    calc["tv_chaikin_osc_3_10"] = ema(adl, 3) - ema(adl, 10)

    # Preserve the locked operation order: flat candles yield a null CLV in
    # CMF even when volume is zero.  Filling before the rolling sum changes
    # the historical null mask.
    cmf_flow = raw_clv * v
    calc["tv_cmf_20"] = cmf_flow.rolling(20, min_periods=20).sum() / v.rolling(20, min_periods=20).sum()
    calc["tv_force_index_13"] = ema(c.diff() * v, 13)

    # Do not algebraically cancel the range term.  A zero range is a locked
    # null boundary and must poison the following 14-row EOM window.
    eom_raw = (hl2 - hl2.shift()) * (h - l).replace(0, np.nan) / v.replace(0, np.nan)
    ...
    calc["tv_pvi"] = pvi
    calc["tv_nvi"] = nvi
    pvt_increments = (v * c.pct_change()).replace([np.inf, -np.inf], np.nan).fillna(0)
    if len(pvt_increments):
        pvt_increments.iloc[0] = 0.0
    calc["tv_pvt"] = pvt_increments.cumsum()
    ...
    calc["tv_close_location_value"] = raw_clv
...
def _normalized_values(series: pd.Series, expected_length: int) -> np.ndarray:
    values = pd.to_numeric(series, errors="coerce").to_numpy(dtype=np.float64, copy=True)
    if len(values) != expected_length:
        raise RuntimeError("TV formula returned the wrong row count")
    values[np.isinf(values)] = np.nan
    # Canonicalize both +0.0 and -0.0 to positive zero.
    values[values == 0.0] = 0.0
    return values


def compute(frame: pd.DataFrame) -> pd.DataFrame:
    """Compute the locked 142 columns for one continuous chronological history.

    OBV, ADL, and PVT are seeded at 0 on the earliest supplied row; PVI and
    NVI are seeded at 1000.  Callers that need pre-live state must prepend
    warmup bars and discard their output only after this function returns.
    """
    ...
    if missing:
        raise ValueError(f"missing TV base columns: {missing}")
    if frame.columns.duplicated().any():
        raise ValueError("frame contains duplicate column names")

    original_index = frame.index
    work = frame.loc[:, list(BASE_COLUMNS)].copy().reset_index(drop=True)
    for name in BASE_COLUMNS:
        work[name] = pd.to_numeric(work[name], errors="coerce").astype(np.float64)

    calculated = _compute_core(work)
    missing_outputs = [name for name in COLUMN_ORDER if name not in calculated]
    extra_outputs = [name for name in calculated if name not in COLUMN_ORDER]
    if missing_outputs or extra_outputs:
        raise RuntimeError(
            f"TV output contract mismatch: missing={missing_outputs}, extra={extra_outputs}"
        )

    arrays = {
        name: _normalized_values(calculated[name], len(work))
        for name in COLUMN_ORDER
    }
    result = pd.DataFrame(arrays, columns=COLUMN_ORDER, index=original_index)
    if tuple(result.columns) != COLUMN_ORDER:
        raise RuntimeError("TV output order drifted")
    if any(dtype != np.dtype("float64") for dtype in result.dtypes):
        raise RuntimeError("TV output dtype drifted")
    return result
