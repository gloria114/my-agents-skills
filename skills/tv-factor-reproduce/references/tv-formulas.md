...
- `EMA(x,N)` 使用 pandas/Pine 等价递推 `ewm(span=N, adjust=False,min_periods=N)`；前 `N-1` 个有效观测保持为空。
...
- `tv_williams_r_14`、`tv_cci_20`、`tv_roc_10/20`、`tv_momentum_10`：标准公式。
- `tv_mfi_14`：保留当前直接相除语义；仅正向资金流时为 `100`，仅负向资金流时为 `0`，正负资金流同时为零时为空。
